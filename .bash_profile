...
alias edit='em'                             # edit:         Opens any file in sublime editor
...
alias wget='curl'                           # wget: Linux version of Server download
...
#   mkpdf:  Combine pdflatex and open pdf into one single command
#   -------------------------------------------------------------------
mkpdf(){

local FILENAME="$1" #| cut -d'.' --complement -f2-
FILENAME=$(echo $FILENAME | cut -f'.' --complement -f2-)
...
#   code fun for open file with VS code
code () {
    if [[ $# = 0 ]]
    then
